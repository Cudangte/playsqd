CREATE TABLE MEDIA_SOURCE
(
    ID INTEGER GENERATED BY DEFAULT AS IDENTITY(START WITH 1, INCREMENT BY 1) primary key,

    NAME VARCHAR(255),
    PATH VARCHAR(255),

    AUTO_SCAN_ON_RESTART BOOLEAN,

    CREATED_BY VARCHAR(255),
    CREATED_DATE TIMESTAMP,
    LAST_MODIFIED_BY VARCHAR(255),
    LAST_MODIFIED_DATE TIMESTAMP
);

CREATE INDEX MEDIA_SOURCE_IDX
    ON MEDIA_SOURCE (NAME, PATH);

CREATE TABLE MEDIA_SOURCE_SCAN_LOG
(
    ID INTEGER GENERATED BY DEFAULT AS IDENTITY(START WITH 1, INCREMENT BY 1) primary key,

    FILES_SCANNED INTEGER,
    FILES_MISSING INTEGER,

    SCAN_DIRECTORY VARCHAR(255),
    SCAN_DURATION_IN_MILLIS INTEGER,
    SCAN_STATUS VARCHAR(255),
    SCAN_STATUS_DETAILS VARCHAR(255),

    OPTION_DELETE_MISSING BOOLEAN,
    OPTION_DELETE_ALL_BEFORE_SCAN BOOLEAN,

    CREATED_BY VARCHAR(255),
    CREATED_DATE TIMESTAMP,
    LAST_MODIFIED_BY VARCHAR(255),
    LAST_MODIFIED_DATE TIMESTAMP
);

CREATE TABLE LIBRARY_ITEM
(
    ID INTEGER GENERATED BY DEFAULT AS IDENTITY(START WITH 1, INCREMENT BY 1) primary key,

    ARTIST_ID VARCHAR(255),
    ARTIST_SPOTIFY_ID VARCHAR(255),
    ARTIST_SPOTIFY_NAME VARCHAR(255),
    ARTIST_NAME VARCHAR(255),
    ARTIST_COUNTRY VARCHAR(255),

    ALBUM_ID VARCHAR(255),
    ALBUM_NAME VARCHAR(255),
    ALBUM_YEAR VARCHAR(255),
    ALBUM_GENRE VARCHAR(255),

    SONG_NAME VARCHAR(1000),
    SONG_TRACK_ID INTEGER,
    SONG_TRACK_LENGTH INTEGER,
    SONG_COMMENT VARCHAR(3000),
    SONG_LYRICS VARCHAR(16777216),

    AUDIO_BIT_RATE VARCHAR(255),
    AUDIO_CHANNEL_TYPE VARCHAR(255),
    AUDIO_ENCODING_TYPE VARCHAR(255),
    AUDIO_SAMPLE_RATE VARCHAR(255),

    FILE_NAME VARCHAR(255),
    FILE_EXTENSION VARCHAR(255),
    FILE_LOCATION VARCHAR(255),

    MISC_PLAY_COUNT INTEGER,
    MISC_IS_FAVORITE BOOLEAN,

    CREATED_BY VARCHAR(255),
    CREATED_DATE TIMESTAMP,
    LAST_MODIFIED_BY VARCHAR(255),
    LAST_MODIFIED_DATE TIMESTAMP
);

CREATE INDEX LIBRARY_ITEM_IDX
	ON LIBRARY_ITEM (ARTIST_ID, ARTIST_NAME, ALBUM_ID, ALBUM_NAME, ALBUM_GENRE, SONG_NAME, FILE_NAME);

